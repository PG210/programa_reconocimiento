...
document.addEventListener("DOMContentLoaded", function() {
    ...
    let label2 = window.label2 || "Datos";
    ...
    if (recmes.length > 0) {
        ...
        new Chart(document.getElementById('trendChart').getContext('2d'), {
            type: 'line',
            data: { labels: labels, datasets: [{ label: label2, data: data, borderColor: '#cdcdcd', backgroundColor: 'rgba(29, 131, 32, 0.2)', fill: true }] }
        });
    ...
    var canvasElement2 = $('#categoryChart-grupo').get(0);
    if (canvasElement2) {  

    new Chart(document.getElementById('categoryChart-grupo').getContext('2d'), {
        type: 'bar',
        data: { labels: labelstotcat, datasets: [{ label: label2, data: datatotcat, backgroundColor: ['#EBB93B', '#79D0F0'] }] }
    });

    }
    
    var canvasElement3 = $('#radarChart').get(0);
    if (canvasElement3) {  
...
if(rectime.length > 0){
    ...
    //obtener todos los meses de enero 1 a diciembre 12
    let mesesTotales = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];

    // Obtener todas las categorías de manera unica
    let categoriasUnicas = [...new Set(rectime.map(item => item.descat))];

    //agregar a la estructura de datos con ceros para cada categoría en cada mes 
    ...
    rectime.forEach(item => {
        let mesIndex = mesesTotales.indexOf(item.mes); 
        let catIndex = categoriasUnicas.indexOf(item.descat);

        if (mesIndex !== -1 && catIndex !== -1) {
            valData[catIndex][mesIndex] += item.total; // Sumar los valores al mes
        }
    });

    // Declarar colores para cada categoría
    const colors = [
...
//validar el canvas
var canvasElement = $('#donutChart').get(0);
if (!canvasElement) {
    return; // Salir del script si no hay canvas
}
...
console.log('datos', grafinsig);
var canvasElement1 = $('#timelineChart11').get(0);
if (!canvasElement1) {
   return; // Salir del script si no hay canvas
}

if (grafinsig.length > 0){
